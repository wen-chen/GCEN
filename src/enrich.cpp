...
void enrich_help() {
  ...
  std::cout
      << "  enrich -e enrichment_gene_list_file -b background_gene_list_file "
         "-g go-basic.obo "
  ...
  std::cout << "examples:\n";
  std::cout << "  enrich -e ../sample_data/enrichment_gene.list -b "
               "../sample_data/background_gene.list "
               "-g ../sample_data/go-basic.obo -a ../sample_data/gene_go.assoc "
               "-p 0.05 -o ../sample_data/enrichment.go\n";
  std::cout << "  enrich -e ../sample_data/enrichment_gene.list -b "
               "../sample_data/background_gene.list "
               "-k ../sample_data/K2ko.tsv -a ../sample_data/gene_kegg.assoc "
               "-p 0.05 -o ../sample_data/enrichment.kegg\n";
}
...
int main(int argc, char* argv[]) {
  ...
  const struct option long_opts[] = {
      ...
      {"enrich", 1, NULL, 'e'}, {"background", 1, NULL, 'b'},
      {"go", 1, NULL, 'g'},     {"kegg", 1, NULL, 'k'},
      {"assoc", 1, NULL, 'a'},  {"pval", 1, NULL, 'p'},
      {"output", 1, NULL, 'o'}, {NULL, 0, NULL, 0}};
  ...
  if (!obo_file_name.empty()) {  // go enrich
    ...
    std::unordered_map<std::string, std::unordered_set<std::string>>
        enrichment_gene_go_map;
    load_gene_list(enrichment_gene_file_name, assoc_map,
                   enrichment_gene_go_map);

    std::unordered_map<std::string, std::unordered_set<std::string>>
        background_gene_go_map;
    load_gene_list(background_gene_file_name, assoc_map,
                   background_gene_go_map);
