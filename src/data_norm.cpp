...
void data_norm_help() {
  ...
  std::cout << "  -m --method <upqt | median | deseq | tmm | hkg> "
               "normalization method (default: upqt)\n";
  std::cout
  ...
  std::cout << "example:\n";
  std::cout << "  data_norm -i ../sample_data/gene_expr.tsv -o "
               "../sample_data/gene_expr_norm.tsv -m tmm\n";
}
...
int main(int argc, char* argv[]) {
  ...
  if (!((method == "upqt") || (method == "median") || (method == "deseq") ||
        (method == "tmm") || (method == "hkg"))) {
  ...
  if (method == "upqt") {
    ...
    std::ifstream housekeeping_gene_file(housekeeping_gene_file_name,
                                         std::ios::in);
