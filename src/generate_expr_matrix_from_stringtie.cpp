...
void stringtie_help() {
  ...
  std::cout << "example:\n";
  std::cout << "  generate_expr_matrix_from_stringtie -i "
               "../sample_data/stringtie/stringtie_sample.txt -o "
               "../sample_data/stringtie/stringtie_gene_expr.tsv\n";
}
...
int main(int argc, char *argv[]) {
  ...
  for (auto item : gene_map) {
    ...
    for (auto transcript_id : transcript_set) {
      std::vector<std::string> transcript_expr_vec =
...
void push_record(
    unsigned int i, std::string &gene_id, std::string &expr_val,
