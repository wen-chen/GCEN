...
void data_filter_help() {
  ...
  std::cout << "  -c --cutoff_mean <number> mean cutoff of gene expression "
               "(default: 0.0)\n";
  std::cout << "  -C --cutoff_sd <number> standard deviation cutoff of gene "
               "expression (default: 0.0)\n";
  std::cout << "  -p --percent_mean <number> keep a proportion of total genes "
               "based mean of gene expression (default: 1.0)\n";
  std::cout << "  -P --percent_sd <number> keep a proportion of total genes "
               "based standard deviation of gene expression (default: 1.0)\n";
  std::cout << "  -v --version display GCEN version\n";
  std::cout << "  -h --help print help information\n";
  std::cout << "example:\n";
  std::cout << "  data_filter -i ../sample_data/gene_expr.tsv -o "
               "../sample_data/gene_expr_filter.tsv -p 0.75\n";
}
...
int main(int argc, char *argv[]) {
  ...
  while (getline(in_file_2, line)) {
    ...
    try {
      ...
      if (mean(value_vector) >= cutoff_mean &&
          deviation(value_vector) >= cutoff_sd) {
        out_file << line << '\n';
      }
    } catch (std::invalid_argument &) {
      continue;
