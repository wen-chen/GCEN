...
void network_merge_help() {
  ...
  std::cout << "example:\n";
  std::cout << "  network_merge -i "
               "../sample_data/test_1.network,../sample_data/test_2.network -o "
               "../sample_data/test_merge.network\n";
}
...
int main(int argc, char* argv[]) {
  ...
  for (std::string& in_file_name : in_file_name_vec) {
    ...
    while (getline(network_file, line)) {
      ...
      if (network.find(gene_a) != network.end() and
  ...
  for (auto const& x : network) {
    for (auto const& y : x.second) {
      ...
      if (positive_cor >= std::abs(negative_cor) and
          positive_cor >= cor_cutoff) {
        out_file << x.first << "\t" << y.first << "\t" << positive_cor << "1"
                 << "\n";
      } else if (std::abs(negative_cor) > positive_cor and
                 std::abs(negative_cor) >= cor_cutoff) {
