...
void module_identify_help() {
  ...
  std::cout << "example:\n";
  std::cout << "  module_identify -i ../sample_data/gene_co_expr.network -o "
               "../sample_data/module.txt\n";
}

int set_intersect(const robin_hood::unordered_set<std::string>& set_a,
                  const robin_hood::unordered_set<std::string>& set_b) {
  int n = 0;
...
void dfs(const std::string& node,
  ...
  if (node_state[node] == 0) {
    module.insert(similarity_network[node].begin(),
                  similarity_network[node].end());
    node_state[node] = 1;
...
void ThreadFunc(
    int n, int thread_num, std::vector<std::string>& node_vec,
    robin_hood::unordered_map<std::string,
                              robin_hood::unordered_set<std::string>>& network,
    double similarity_cutoff, std::vector<edeg_similarity>& similarity_vec) {
  ...
  for (int i = start; i < stop; ++i) {
    for (int j = i + 1; j < node_num; ++j) {
      ...
      double similarity =
          (shared + a_b) / (node_a_degree + node_b_degree - shared - a_b);
      if (similarity > similarity_cutoff) {
        local_similarity_vec.push_back(
            edeg_similarity{node_a, node_b, similarity});
      }
    }
  }
  ...
  mutex_lock.lock();
  similarity_vec.insert(similarity_vec.end(), local_similarity_vec.begin(),
                        local_similarity_vec.end());
  mutex_lock.unlock();
}
...
int main(int argc, char* argv[]) {
  ...
  const struct option long_opts[] = {
      ...
      {"similarity", 1, NULL, 's'}, {"pval", 1, NULL, 'p'},
  ...
  for (int i = 0; i < thread_num; ++i) {
    threads.push_back(std::thread{ThreadFunc, i, thread_num, std::ref(node_vec),
                                  std::ref(network), similarity_cutoff,
