...
inline double logHypergeometricProb(double* logFacs, const int a, const int b,
...
double fisher_exact(int a, int b, int c, int d) {
  int n = a + b + c + d;
  double* logFacs =
      new double[n + 1];    // *** dynamically allocate memory logFacs[0..n] ***
  initLogFacs(logFacs, n);  // *** initialize logFacs array ***
  double logpCutoff =
      logHypergeometricProb(logFacs, a, b, c, d);  // *** logFacs added
  double pFraction = 0;
  for (int x = 0; x <= n; ++x) {
    if (a + b - x >= 0 && a + c - x >= 0 && d - a + x >= 0) {
      double l =
          logHypergeometricProb(logFacs, x, a + b - x, a + c - x, d - a + x);
      if (l <= logpCutoff) {
...
double calc_p_hypergeometric(int pop_n, int pop_count, int study_n,
                             int study_count) {
  int a = study_count;
