...
double calc_weighted_factor(const std::vector <double>& obs, const std::vector <double>& ref,
    double logratio_trim = 0.3, double sum_trim = 0.05, double cutoff = -1e+10) {
  double obs_sum = 0.0;
  for (auto& n : obs)
    obs_sum += n;
  ...
  for (unsigned int i = 0; i < obs.size(); ++i) {
    if ((obs[i] - 0.0 > 1.0e-30) && (ref[i] - 0.0 > 1.0e-30)) {
      if ((log2(obs[i] / obs_sum) + log2(ref[i] / ref_sum)) / 2 > cutoff) {
        ...
        v.push_back((obs_sum - obs[i]) / obs_sum / obs[i] + (ref_sum - ref[i]) / ref_sum / ref[i]);
        n += 1.0;
      }
    }
  }
  ...
  for (unsigned int i = 0; i < log_r.size(); ++i) {
    if ((log_r_rank[i] >= low_l) && (log_r_rank[i] <= high_l) && (abs_e_rank[i] >= low_s) && (abs_e_rank[i] <= high_s)) {
...
void norm_deseq(std::vector <std::vector <double>> & matrix) {
  ...
  // 对比例数矩阵的每一列求中位数。
  std::vector <std::vector <double>> matrix_ratio_t = matrix_transpose(matrix_ratio);
  std::vector <double> median_vec;
