...
void rwr_help() {
  ...
  std::cout << "  -d --directed_network the input network is directed "
               "(defalut: the input network is undirected)\n";
  std::cout
      << "  -w --weighted_network the edge weights of network will be "
         "considered (defalut: all edge weights of network are set to 1.0)\n";
  std::cout
      << "  -W --weighted_gene the weights of seed genes will be considered "
         "(defalut: all weights of seed genes are set to 1.0)\n";
  std::cout << "  -v --version display GCEN version\n";
  std::cout << "  -h --help print help information\n";
  std::cout << "example:\n";
  std::cout << "  rwr -n ../sample_data/gene_co_expr.network -g "
               "../sample_data/rwr_seed_genes.list -o "
               "../sample_data/rwr_ranked_gene.tsv\n";
}
...
int main(int argc, char* argv[]) {
  ...
  robin_hood::unordered_map<std::string,
      ...
      network;
  load_network(network_file_name, if_directed_network, if_weighted_network,
               gene_vec, network);
  ...
  int step = rwr(matrix, P0, gamma, PT);
