...
void go_annot_help() {
  ...
  std::cout << "  -m --module <module file> (if -n is specified, the -m is ignored)\n";
  ...
  std::cout << "examples:\n";
  std::cout << "  ./annotate -g ../sample_data/go-basic.obo -a ../sample_data/gene_go.assoc "
               "-n ../sample_data/gene_co_expr.network -o ../sample_data/network_go_annotation\n";
  std::cout << "  ./annotate -g ../sample_data/go-basic.obo -a ../sample_data/gene_go.assoc "
               "-m ../sample_data/module.txt -o ../sample_data/module_go_annotation\n";
  std::cout << "  ./annotate -k ../sample_data/K2ko.tsv -a ../sample_data/gene_kegg.assoc "
               "-n ../sample_data/gene_co_expr.network -o ../sample_data/network_kegg_annotation\n";
  std::cout << "  ./annotate -k ../sample_data/K2ko.tsv -a ../sample_data/gene_kegg.assoc "
               "-m ./sample_data/module.txt -o ../sample_data/module_kegg_annotation\n";
}


void network_go_annotate(int n, int thread_num, std::vector <std::string> & gene_vec, 
    ...
    std::unordered_map <std::string, int> & background_count_map, int pop_n, double pval_cutoff,
    std::string & out_dir);


void module_go_annotate(int n, int thread_num, std::vector <std::unordered_set <std::string>> & module_vec,
    std::unordered_map <std::string, std::unordered_set <std::string>> & assoc_map,
    std::unordered_map <std::string, GO_term> & go_term_map,
    std::unordered_map <std::string, int> & background_count_map, int pop_n, double pval_cutoff,
    std::string & out_dir );
...
void network_kegg_annotate(int n, int thread_num,std::vector <std::string> & gene_vec,
    std::unordered_map <std::string, std::unordered_set <std::string>> & network,
    std::unordered_map <std::string, std::unordered_set <std::string>> & assoc_map,
...
void module_kegg_annotate(int n, int thread_num, std::vector <std::unordered_set <std::string>> & module_vec,
    std::unordered_map <std::string, std::unordered_set <std::string>> & assoc_map,
    std::unordered_map <std::string, std::string> ko_map,
    std::unordered_map <std::string, int> & background_count_map, int pop_n, double pval_cutoff,
    std::string & out_dir );
...
int main(int argc, char* argv[]) {
  ...
  if (network_file_name.empty() and module_file_name.empty()) {
    std::cerr << "Error: -n/--network or -m/--module is required but not specified!\n";
    return -1;
  }
  ...
  if (!obo_file_name.empty()) { // go annotate
    ...
    if (!network_file_name.empty()) {
      ...
      for (int i = 0; i < thread_num; ++i) {
        threads.push_back(std::thread{ network_go_annotate, i, thread_num, std::ref(gene_vec), std::ref(network), std::ref(assoc_map),
        std::ref(go_term_map), std::ref(background_count_map), pop_n, pval_cutoff, std::ref(out_dir)});
      }
      ...
      std::vector <std::thread> threads;
      for (int i = 0; i < thread_num; ++i) {
        threads.push_back(std::thread{ module_go_annotate, i, thread_num, std::ref(module_vec), std::ref(assoc_map),
        std::ref(go_term_map), std::ref(background_count_map), pop_n, pval_cutoff, std::ref(out_dir)});
      }
    ...
    if (!network_file_name.empty()) {
      ...
      std::vector <std::thread> threads;
      for (int i = 0; i < thread_num; ++i) {
        threads.push_back(std::thread{ network_kegg_annotate, i, thread_num, std::ref(gene_vec), std::ref(network), std::ref(assoc_map),
        std::ref(ko_map), std::ref(background_count_map), pop_n, pval_cutoff, std::ref(out_dir)});
      }
      ...
      std::vector <std::thread> threads;
      for (int i = 0; i < thread_num; ++i) {
        threads.push_back(std::thread{ module_kegg_annotate, i, thread_num, std::ref(module_vec), std::ref(assoc_map),
        std::ref(ko_map), std::ref(background_count_map), pop_n, pval_cutoff, std::ref(out_dir)});
      }
...
void module_go_annotate(int n, int thread_num, std::vector <std::unordered_set <std::string>> & module_vec,
  ...
  for (int i = start; i < stop; ++i) {
    ...
    if (!GO_result_vec.empty()) {
      ...
      std::ofstream module_file(out_dir + "/module_" + std::to_string(i) + ".txt", std::ios::out);
      for (auto & gene : module) {
        module_file << gene << '\n';
      }
      module_file.close();

      std::ofstream result_file(out_dir + "/module_" + std::to_string(i) + ".go", std::ios::out);

      result_file << "id\tname\tname_space\tstudy_count\tstudy_n\tpop_count\tpop_n\tp_val\n";

      for (auto & result : GO_result_vec) { 
        result_file << result.id << '\t' << result.name << '\t' << result.name_space << '\t' <<
        result.study_count << '\t' << result.study_n << '\t' << result.pop_count << '\t' <<
...
void module_kegg_annotate(int n, int thread_num, std::vector <std::unordered_set <std::string>> & module_vec,
  ...
  for (int i = start; i < stop; ++i) {
    ...
    if (!ko_result_vec.empty()) {
      ...
      std::ofstream result_file(out_dir + "/module_" + std::to_string(i) + ".kegg", std::ios::out);
